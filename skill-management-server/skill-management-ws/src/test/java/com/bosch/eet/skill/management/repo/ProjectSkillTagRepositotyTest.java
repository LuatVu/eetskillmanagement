...
@DataJpaTest
@ActiveProfiles("dev")
public class ProjectSkillTagRepositotyTest {
	...
	void prepare() throws ParseException {
		...
		ProjectType projectType = ProjectType.builder().id("b8987dfa-f6e2-99d3-f511-3df3286bd7c0").name("Bosch")
				.build();
		customer = Customer.builder().name("Data jpa test").build();
	...
	void findProjectsByCustomerIdAndSkillTagName() {
		List<ProjectDto> projectDtos = projectSkillTagRepository
				.findProjectsByCustomerIdAndSkillTagName(customer.getId(), skillTag1.getName());

		assertThat(projectDtos.size()).isEqualTo(1);
		assertThat(projectDtos.get(0).getName()).isEqualTo(project.getName());   
		projectDtos = projectSkillTagRepository.findProjectsByCustomerIdAndSkillTagName(customer.getId(),
				skillTag2.getName());
		assertThat(projectDtos.size()).isEqualTo(1);
		 assertThat(projectDtos.get(0).getName()).isEqualTo(project.getName());   
	}
